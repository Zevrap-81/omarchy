...
Item {
  ...
  Variants {
    ...
    PanelWindow {
      ...
      // This surface is exclusionMode: Ignore, so it spans the whole output and
      // knows nothing about a monitor's reserved area. The stock top margin is
      // just the bar's own height, which lands the toast column inside any
      // custom reserved strip (see monitors.lua -> reserved_area) rather than
      // below it. Read the compositor's reserved edges and clear those instead.
      //
      // Hyprland's reserved array is [left, top, right, bottom] and already
      // includes the bar's exclusive zone, so on a monitor without a custom
      // reserved area this resolves to exactly the stock margins.
      readonly property var hyprReserved: {
        var monitor = Hyprland.monitorFor(popupWindow.screen)
        var ipc = monitor ? monitor.lastIpcObject : null
        var reserved = ipc ? ipc.reserved : null
        return (reserved && reserved.length >= 4) ? reserved : null
      }

      function reservedEdge(index) {
        var reserved = popupWindow.hyprReserved
        if (!reserved) return 0
        var value = Number(reserved[index])
        return isFinite(value) && value > 0 ? value : 0
      }

      readonly property int popupTopMargin: Math.max(
        popupPlacement.margins.top, reservedEdge(1) + Style.gapsOut)
      readonly property int popupRightMargin: Math.max(
        popupPlacement.margins.right, reservedEdge(2) + Style.gapsOut)
      ...
      ColumnLayout {
        ...
        anchors.topMargin: popupWindow.popupTopMargin
